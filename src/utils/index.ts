...
const abiBSC = [
  {
    inputs: [
      ...
      { internalType: 'address', name: '_NUS_WETH', type: 'address' },
      { internalType: 'address', name: '_lpRewards', type: 'address' }
    ],
  ...
  },
  {
    inputs: [],
    name: 'lpRewards',
    outputs: [{ internalType: 'contract ILPRewards', name: '', type: 'address' }],
    stateMutability: 'view',
    type: 'function'
  },
  {
...
// shorten the checksummed version of the input address to have 0x + 4 characters at start and end
export function shortenWrapHistoryAddress(address: string, chars = 12): string {
  const parsed = isAddress(address)
  if (!parsed) {
    throw Error(`Invalid 'address' parameter '${address}'.`)
  }
  return `${parsed.substring(0, chars + 2)}`
}
...
export function getContract(address: string, ABI: any, library: Web3Provider, account?: string): Contract {
  ...
  const contr = new Contract(address, ABI, getProviderOrSigner(library, account) as any)
  return contr
}
...
export function getRouterContract(chainId: number, library: Web3Provider, account?: string): Contract {
  ...
  return getContract(
    SELECTED_ROUTER,
    chainId == 97 ? abiBSC : chainId == 56 ? abiBSCmainnet : IUniswapV2Router02ABI,
    library,
    account
  )
}
